...
public:
   ...
   */
    RingBuffer(size_t capacity = n) : buffer(reinterpret_cast<T *>(Memory::alignedMalloc(capacity * sizeof(T)))),
                                      allocated(capacity)
    {
    }
   ...
   */
    RingBuffer(const RingBuffer &other) : buffer(reinterpret_cast<T *>(Memory::alignedMalloc(other.allocated * sizeof(T)))),
